...
def query_users(sql_query: str):
    """
    Executes the given SQL query on the users table and returns the results.
    Table: users
...
def get_user_transcripts(user_id: int) -> str:
    """
    Extracts and returns all transcripts for a given user_id from the customer_transcripts.json as one long string.
    """
...
customer_research_agent = Agent(
    ...
    instructions=(
        ...
        "Give a customer ID, you should create a customer report that:"
            "- retrieves customer details"
            "- reads previous customer transcripts on the customer interests, to be used to personalize emails"
            "- summarized latest news (search the web) on things related to their interests they're noted in the tanscript"
    ),
    tools=[query_users, get_user_transcripts, web_search_tool],
)
